...
export const useFriendsStore = create<FriendsState>((set, get) => ({
  ...
  searchUsers: async (query: string): Promise<User[]> => {
    try {
      ...
      const filteredUsers = (users || []).filter((u: any) => {
        ...
        if (isSelf) console.log(`Filtered out (self): ${u.username}`);
        if (isFriend) console.log(`Filtered out (friend): ${u.username}`);
        if (isBlocked) console.log(`Filtered out (blocked): ${u.username}`);
        if (hasRequest) console.log(`Filtered out (pending request): ${u.username}`);

        return !isSelf && !isFriend && !isBlocked && !hasRequest;
      });

      console.log('Filtered search results:', filteredUsers.length, filteredUsers.map(u => u.username));
  ...
  getSuggestions: async (): Promise<User[]> => {
    try {
      ...
      console.log('Filtered suggestions:', filteredSuggestions.length, filteredSuggestions.map(u => u.username));
