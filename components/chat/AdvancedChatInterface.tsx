...
export const AdvancedChatInterface: React.FC<AdvancedChatInterfaceProps> = ({ chat, onBack }) => {
  ...
  const typingStatusHook = useTypingStatus(chat.id, user?.id);
  const typing = typingStatusHook.typing;
  const setTypingStatus = typingStatusHook.setTypingStatus;
  const seenDeliveredHook = useSeenDelivered(chat.id, user?.id) as any;
  const seenStatus = seenDeliveredHook.seenStatus;
  const deliveredStatus = seenDeliveredHook.deliveredStatus;
  ...
  const giphyHook = useGiphy();
  const giphyResults = giphyHook.giphyResults;
  const searchGiphy = giphyHook.searchGiphy;
  const selectGif = giphyHook.selectGif;
  const chatHeaderData = useChatHeaderData(chat, user?.id);
  const headerUser = chatHeaderData.headerUser;
  const headerAvatar = chatHeaderData.headerAvatar;
  const lastSeen = chatHeaderData.lastSeen;
  const { isMuted, setMute } = useMuteChat(chat.id, user?.id);
  const pollsHook = usePolls(chat.id, user?.id);
  const polls = pollsHook.polls;
  const votes = pollsHook.votes;
  const createPoll = pollsHook.createPoll;
  const votePoll = pollsHook.votePoll;
  ...
  const handleVoiceEffectSend = async () => {
    ...
    setShowVoiceEffectModal(false);
      ...
      await sendMessage(chat.id, publicURL, 'voice', false);
      setShowVoiceEffectModal(false);
      setPendingVoiceUri(null);
  ...
  const playVoiceWithEffect = async (audioUrl: string, effectKey: string) => {
    const effect = Array.isArray(VOICE_EFFECTS) ? VOICE_EFFECTS.find(e => e.key === effectKey) || VOICE_EFFECTS[0] : VOICE_EFFECTS[0];
    if (voicePreviewSound) {
      await voicePreviewSound.unloadAsync();
      setVoicePreviewSound(null);
    }
  ...
  // Track if the theme modal was opened for this chat
  const [modalWasOpened, setModalWasOpened] = useState(false);
  // Track recent customization updates to debounce backend/store overwrites
  const justUpdatedCustomization = useRef<{ [chatId: string]: number }>({});

  // Only sync local customization state from chat.customization when chat.id changes or modal is first opened
  useEffect(() => {
    // Only sync if not just updated in the last 2 seconds
    if (
      !justUpdatedCustomization.current[chat.id] ||
      Date.now() - justUpdatedCustomization.current[chat.id] > 2000
    ) {
  ...
  }, [chat.id, chat.customization]);

  // When the modal is opened, set modalWasOpened to true
  useEffect(() => {
    if (showThemeModal) setModalWasOpened(true);
    else setModalWasOpened(false);
  }, [showThemeModal]);

  // 2. Permission: Only admins/owners can save in group chats
  const isGroup = chat.type === 'group';
  const myRole = user ? (Array.isArray(chat.members) ? (chat.members.find((m: any) => m.userId === user.id)?.role || 'member') : 'member') : 'member';
  ...
  const getUserInfo = (userId: string) => Array.isArray(chatMembers) ? chatMembers.find(u => u.id === userId) || { displayName: 'User', avatar: '' } : { displayName: 'User', avatar: '' };
  ...
  const renderMessage = ({ item }: { item: any }) => {
    ...
    return (
      <MessageBubble
        ...
        onPlayVoice={() => playVoiceWithEffect(item.content, item.voice_effect || 'normal')}
  ...
  const prevCount = useRef(safeMessages.length);
  useEffect(() => {
    if (safeMessages.length !== prevCount.current) {
    console.log('[Chat] Rendering', safeMessages.length, 'messages:', safeMessages.map(m => m.id));
      prevCount.current = safeMessages.length;
    }
  }, [safeMessages.length]);
  ...
  // Log when a new message arrives
  ...
  useEffect(() => {
    if (chatMessages.length > prevMessageCount.current) {
      ...
      newMessages.forEach(msg => {
        console.log('[Chat Event] New message:', msg);
      });
      prevMessageCount.current = chatMessages.length;
    }
  }, [chatMessages.length]);

  // Log when a new poll is created
  const prevPollCount = useRef(polls.length);
  useEffect(() => {
    if (polls.length > prevPollCount.current) {
      const newPolls = polls.slice(prevPollCount.current);
      newPolls.forEach(poll => {
        console.log('[Chat Event] New poll created:', poll);
      });
      prevPollCount.current = polls.length;
    }
  }, [polls.length]);

  // Log when a new user starts typing
  const prevTyping = useRef(typing.length);
  useEffect(() => {
    if (typing.length > prevTyping.current) {
      const newTypers = typing.slice(prevTyping.current);
      newTypers.forEach(userId => {
        console.log('[Chat Event] User started typing:', userId);
      });
      prevTyping.current = typing.length;
    } else if (typing.length < prevTyping.current) {
      prevTyping.current = typing.length;
    }
  }, [typing.length]);
  
  // Helper to optimistically update chat customization in parent chat object
  const optimisticUpdateChatCustomization = (newCustomization: any) => {
    if (chat && typeof chat === 'object') {
      chat.customization = { ...newCustomization };
    }
  };

  // Helper to re-fetch chat from Supabase after update
  const refetchChatCustomization = async () => {
    const { data, error } = await supabase
      .from('chats')
    ...
    if (!error && data?.customization) {
      ...
      setSelectedTheme(data.customization.theme || 'classic');
      setSelectedBackground(data.customization.background || '');
      setSelectedBubbleStyle(data.customization.bubbleStyle || 'rounded');
      setSelectedFontFamily(data.customization.fontFamily || 'System');
      setSelectedFontSize(data.customization.fontSize || 'medium');
    }
  };
  ...
  return (
    ...
    >
      ...
      <View style={styles.header}>
        ...
        <TouchableOpacity
          onPress={async () => {
            const receiverId = Array.isArray(chat.members) ? chat.members.find(m => m.userId !== user.id)?.userId : undefined;
            if (receiverId) {
              try {
                const { data: channelId, error } = await supabase.rpc('start_call', { caller: user.id, receiver: receiverId, call_type: 'audio' });
                if (error) throw error;
                // Fetch the call row by channel_id to get the UUID id and call object
                ...
                if (!callError && callRow) {
                  ...
                  // Fallback: show CallInterface with dummy call to trigger 'Calling Not Available' in Expo Go
                  setActiveCall({
                    id: 'dummy',
                    caller_id: user.id,
        ...
        <TouchableOpacity
          onPress={async () => {
            ...
            if (receiverId) {
              try {
                const { data: channelId, error } = await supabase.rpc('start_call', { caller: user.id, receiver: receiverId, call_type: 'video' });
                if (error) throw error;
                // Fetch the call row by channel_id to get the UUID id and call object
                const { data: callRow, error: callError } = await supabase
                  .from('calls')
      ...
      <Animated.View style={{ flex: 1, transform: [{ translateY: slideAnim }] }}>
        <MessageList
          ...
          onSwipeReply={(message: any) => setReplyTo(message)}
          onPlayVoice={(url: string, effect?: string) => playVoiceWithEffect(url, effect || 'normal')}
              ...
              <FlatList
                ...
                renderItem={({ item }) => (
                  <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 8 }}>
                    ...
                    <ThemedText>{item.displayName || 'Unknown User'}</ThemedText>
                  </View>
                )}
      ...
      {showThemeModal && (
        <Modal visible={showThemeModal} animationType="fade" transparent onRequestClose={() => setShowThemeModal(false)}>
          <View style={{ flex: 1, backgroundColor: 'rgba(240,240,245,0.85)', justifyContent: 'center', alignItems: 'center' }}>
            <BlurView intensity={40} tint="light" style={{ ...StyleSheet.absoluteFillObject, zIndex: 0 }} />
            <Animated.View style={{
              width: '96%',
              maxWidth: 340, // was 420
              backgroundColor: 'rgba(255,255,255,0.82)',
              borderRadius: 22, // was 32
              padding: 0,
              overflow: 'hidden',
              shadowColor: '#b0c4de',
              shadowOpacity: 0.22,
              shadowRadius: 24, // was 36
              elevation: 16, // was 24
              borderWidth: 1,
              borderColor: 'rgba(200,200,220,0.22)',
              marginTop: 4,
              marginBottom: 4,
              alignItems: 'stretch',
              zIndex: 1,
              ...
              {/* Header */}
              <View style={{ padding: 16, paddingBottom: 6, alignItems: 'center', backgroundColor: 'rgba(255,255,255,0.18)', borderTopLeftRadius: 22, borderTopRightRadius: 22, shadowColor: '#e0e7ef', shadowOpacity: 0.18, shadowRadius: 8, elevation: 1 }}>
                <ThemedText style={{ fontWeight: 'bold', fontSize: 17, marginBottom: 1, textAlign: 'center', letterSpacing: 1.1, color: '#232526', textShadowColor: '#e0e7ef', textShadowOffset: { width: 0, height: 1 }, textShadowRadius: 4 }}>Chat Theme & Background</ThemedText>
                <ThemedText style={{ fontSize: 11, color: '#888', marginBottom: 4, fontWeight: '500' }}>Personalize your chat look</ThemedText>
              </View>
              {/* Theme Row */}
              <View style={{ marginHorizontal: 10, marginBottom: 8 }}>
                <ThemedText style={{ fontWeight: '700', fontSize: 12, color: '#00bfae', marginBottom: 4, letterSpacing: 0.2 }}>Theme</ThemedText>
                <View style={{ flexDirection: 'row', flexWrap: 'wrap' }}>
                  {builtInThemes.map(theme => (
                    <TouchableOpacity key={theme.key} style={{ marginRight: 6, marginBottom: 6, borderWidth: selectedTheme === theme.key ? 2 : 0, borderColor: '#00bfae', borderRadius: 8, padding: 6, minWidth: 36, backgroundColor: selectedTheme === theme.key ? 'rgba(0,191,174,0.13)' : '#f7fafd', alignItems: 'center' }} onPress={async () => {
                      ...
                      setCustomization(newCustomization);
                      optimisticUpdateChatCustomization(newCustomization);
                      await supabase.from('chats').update({ customization: newCustomization }).eq('id', chat.id);
                      useChatStore.getState().updateChatCustomization(chat.id, newCustomization);
                      justUpdatedCustomization.current[chat.id] = Date.now();
                      setTimeout(refetchChatCustomization, 400);
                    }}>
                      <Ionicons name={theme.key === 'classic' ? 'color-palette-outline' : theme.key === 'dark' ? 'moon-outline' : theme.key === 'gradient' ? 'color-filter-outline' : theme.key === 'glass' ? 'cube-outline' : theme.key === 'neon' ? 'flash-outline' : theme.key === 'cyberpunk' ? 'hardware-chip-outline' : 'ellipse-outline'} size={15} color={selectedTheme === theme.key ? '#00bfae' : '#888'} />
                      <ThemedText style={{ fontSize: 8, color: selectedTheme === theme.key ? '#00bfae' : '#232526', marginTop: 2 }}>{theme.name}</ThemedText>
                    </TouchableOpacity>
                  ))}
                </View>
              </View>
              {/* Background Row */}
              <View style={{ marginHorizontal: 10, marginBottom: 8 }}>
                <ThemedText style={{ fontWeight: '700', fontSize: 12, color: '#ffb300', marginBottom: 4, letterSpacing: 0.2 }}>Background</ThemedText>
                <View style={{ flexDirection: 'row', flexWrap: 'wrap' }}>
                  {builtInBackgrounds.map(bg => (
                    <TouchableOpacity key={bg.key} style={{ marginRight: 6, marginBottom: 6, borderWidth: selectedBackground === bg.value ? 2 : 0, borderColor: '#ffb300', borderRadius: 8, padding: 6, minWidth: 36, backgroundColor: selectedBackground === bg.value ? 'rgba(255,179,0,0.13)' : '#f7fafd', alignItems: 'center' }} onPress={async () => {
                      ...
                      <View style={{ width: 15, height: 15, borderRadius: 4, backgroundColor: bg.type === 'color' ? bg.value : '#eee', borderWidth: 1, borderColor: '#ddd', marginBottom: 2 }} />
                      <ThemedText style={{ fontSize: 8, color: selectedBackground === bg.value ? '#ffb300' : '#232526' }}>{bg.name}</ThemedText>
                    </TouchableOpacity>
                  ))}
                  <TouchableOpacity style={{ marginRight: 6, marginBottom: 6, borderWidth: 0, borderRadius: 8, padding: 6, alignItems: 'center', backgroundColor: '#f7fafd', minWidth: 36 }} onPress={handlePickCustomImage} disabled={uploading}>
                    <Ionicons name="cloud-upload-outline" size={15} color="#888" />
                    <ThemedText style={{ fontSize: 8, color: '#888' }}>Upload</ThemedText>
                  </TouchableOpacity>
                </View>
              </View>
              {/* Bubble Style Row */}
              <View style={{ marginHorizontal: 10, marginBottom: 8 }}>
                <ThemedText style={{ fontWeight: '700', fontSize: 12, color: '#00bfae', marginBottom: 4, letterSpacing: 0.2 }}>Bubble Style</ThemedText>
                <View style={{ flexDirection: 'row', flexWrap: 'wrap' }}>
                  {bubbleStyles.map(bs => (
                    <TouchableOpacity key={bs.key} style={{ marginRight: 6, marginBottom: 6, borderWidth: selectedBubbleStyle === bs.key ? 2 : 0, borderColor: '#00bfae', borderRadius: 8, padding: 6, backgroundColor: selectedBubbleStyle === bs.key ? 'rgba(0,191,174,0.13)' : '#f7fafd', alignItems: 'center', minWidth: 36 }} onPress={async () => {
                      setSelectedBubbleStyle(bs.key);
                      const newCustomization = {
                        theme: selectedTheme,
                        background: selectedBackground,
                        bubbleStyle: bs.key,
                      ...
                      <Ionicons name={bs.key === 'rounded' ? 'chatbubble-ellipses-outline' : bs.key === 'gradient' ? 'color-filter-outline' : 'cube-outline'} size={15} color={selectedBubbleStyle === bs.key ? '#00bfae' : '#888'} />
                      <ThemedText style={{ fontSize: 8, color: selectedBubbleStyle === bs.key ? '#00bfae' : '#232526', marginTop: 2 }}>{bs.name}</ThemedText>
                    </TouchableOpacity>
                  ))}
                </View>
              </View>
              {/* Font Row */}
              <View style={{ marginHorizontal: 10, marginBottom: 8 }}>
                <ThemedText style={{ fontWeight: '700', fontSize: 12, color: '#ffb300', marginBottom: 4, letterSpacing: 0.2 }}>Font</ThemedText>
                <View style={{ flexDirection: 'row', flexWrap: 'wrap' }}>
                  {fontFamilies.map(ff => (
                    <TouchableOpacity key={ff} style={{ marginRight: 6, marginBottom: 6, borderWidth: selectedFontFamily === ff ? 2 : 0, borderColor: '#ffb300', borderRadius: 8, padding: 6, backgroundColor: selectedFontFamily === ff ? 'rgba(255,179,0,0.13)' : '#f7fafd', alignItems: 'center', minWidth: 36 }} onPress={async () => {
                      setSelectedFontFamily(ff);
                      const newCustomization = {
                        theme: selectedTheme,
                        background: selectedBackground,
                        bubbleStyle: selectedBubbleStyle,
                      ...
                      <Ionicons name="text-outline" size={15} color={selectedFontFamily === ff ? '#ffb300' : '#888'} />
                      <ThemedText style={{ fontSize: 8, color: selectedFontFamily === ff ? '#ffb300' : '#232526', marginTop: 2 }}>{ff}</ThemedText>
                    </TouchableOpacity>
                  ))}
                </View>
              </View>
              {/* Font Size Row */}
              <View style={{ marginHorizontal: 10, marginBottom: 8 }}>
                <ThemedText style={{ fontWeight: '700', fontSize: 12, color: '#00bfae', marginBottom: 4, letterSpacing: 0.2 }}>Font Size</ThemedText>
                <View style={{ flexDirection: 'row', flexWrap: 'wrap' }}>
                  {fontSizes.map(fs => (
                    <TouchableOpacity key={fs} style={{ marginRight: 6, marginBottom: 6, borderWidth: selectedFontSize === fs ? 2 : 0, borderColor: '#00bfae', borderRadius: 8, padding: 6, backgroundColor: selectedFontSize === fs ? 'rgba(0,191,174,0.13)' : '#f7fafd', alignItems: 'center', minWidth: 36 }} onPress={async () => {
                      ...
                      <Ionicons name={fs === 'small' ? 'remove-outline' : fs === 'large' ? 'add-outline' : 'ellipsis-horizontal-outline'} size={15} color={selectedFontSize === fs ? '#00bfae' : '#888'} />
                      <ThemedText style={{ fontSize: 8, color: selectedFontSize === fs ? '#00bfae' : '#232526', marginTop: 2 }}>{fs.charAt(0).toUpperCase() + fs.slice(1)}</ThemedText>
                    </TouchableOpacity>
                  ))}
                </View>
              </View>
              {/* Live Preview Card */}
              <View style={{ marginHorizontal: 10, marginBottom: 10, backgroundColor: 'rgba(255,255,255,0.55)', borderRadius: 14, padding: 10, alignItems: 'center', shadowColor: '#e0e7ef', shadowOpacity: 0.16, shadowRadius: 6, elevation: 2, borderWidth: 1, borderColor: 'rgba(200,200,220,0.18)', overflow: 'hidden', minHeight: 60 }}>
                <BlurView intensity={30} tint="light" style={{ ...StyleSheet.absoluteFillObject, borderRadius: 14, zIndex: 0 }} />
                <ThemedText style={{ color: selectedTheme === 'dark' ? '#fff' : '#232526', fontFamily: selectedFontFamily, fontSize: selectedFontSize === 'small' ? 10 : selectedFontSize === 'large' ? 15 : 12, letterSpacing: 0.5, marginBottom: 4, fontWeight: '600', zIndex: 1 }}>
                  This is a preview of your chat theme!
                </ThemedText>
                <View style={{ marginTop: 4, flexDirection: 'row', alignItems: 'flex-end', width: '100%', justifyContent: 'center', zIndex: 1 }}>
                  <View style={{ width: '60%', backgroundColor: selectedTheme === 'dark' ? '#232526' : selectedTheme === 'gradient' ? 'linear-gradient(90deg, #ff9966, #ff5e62)' : selectedTheme === 'pastel' ? '#a8edea' : selectedTheme === 'glass' ? 'rgba(255,255,255,0.7)' : selectedTheme === 'neon' ? '#00FFF7' : selectedTheme === 'cyberpunk' ? '#ff00ea' : '#25d366', borderRadius: selectedBubbleStyle === 'rounded' ? 10 : 4, padding: 7, shadowColor: selectedTheme === 'neon' ? '#00FFF7' : selectedTheme === 'cyberpunk' ? '#ff00ea' : '#25d366', shadowOpacity: 0.22, shadowRadius: 6, elevation: 2, borderWidth: 1, borderColor: 'rgba(200,200,220,0.18)' }}>
                    <ThemedText style={{ color: selectedTheme === 'dark' ? '#fff' : selectedTheme === 'neon' ? '#232526' : selectedTheme === 'cyberpunk' ? '#fff' : '#222', fontFamily: selectedFontFamily, fontWeight: '500', fontSize: 11 }}>Hello! 👋</ThemedText>
                  </View>
                </View>
              </View>
              {/* Only admins/owners can edit notice */}
              {!canEditTheme && (
                <ThemedText style={{ color: '#888', marginTop: 6, fontSize: 10, textAlign: 'center' }}>Only admins/owners can change the theme in group chats.</ThemedText>
              )}
            </Animated.View>
          </View>
        </Modal>
