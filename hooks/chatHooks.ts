...
export function usePinnedMessage(chatId: string, userId?: string) {
  const [pinnedMessage, setPinnedMessageState] = useState<any>(null);

  const setPinned = useCallback(async (message: any) => {
    try {
      await supabase
        .from('chats')
        .update({ pinned_message_id: message?.id || null })
        .eq('id', chatId);
      
      setPinnedMessageState(message);
    } catch (error) {
