...
-- COMPLETE MEAZ APP DATABASE SCHEMA (MERGED)
-- This migration merges all features, tables, triggers, functions, indexes, and RLS policies from both previous migrations.
-- It ensures all features are present, robust, and non-duplicated.
...
-- === ONE-TIME DATA FIXES ===
-- Backfill missing users from auth.users into public.users with unique username logic
...
-- Trim usernames and display names
UPDATE public.users 
SET 
...
-- === ROBUST USER PROFILE CREATION TRIGGER/FUNCTION ===
...
BEGIN
  IF NOT EXISTS (SELECT 1 FROM public.users WHERE id = NEW.id) THEN
    INSERT INTO public.users (
      ...
      trim(COALESCE(NEW.raw_user_meta_data->>'display_name', split_part(NEW.email, '@', 1))),
      now(),
      false
    )
    ON CONFLICT (username) DO NOTHING;
...
CREATE TABLE IF NOT EXISTS public.chat_members (
  ...
  pinned_message_id uuid REFERENCES public.messages(id),
  updated_at timestamptz DEFAULT now(),
...
CREATE TABLE IF NOT EXISTS public.messages (
  ...
  original_sender_id uuid REFERENCES public.users(id),
  translation jsonb,
  encryption_key text,
  is_encrypted boolean DEFAULT false,
  audio_url text,
...
BEGIN
  ...
  WHERE u1.id = first_user AND u2.id = second_user;

  -- Create new chat
  INSERT INTO public.chats (name, type, created_by)
  VALUES (COALESCE(chat_name, 'Direct Chat'), 'direct', auth.uid())
...
-- Auto-create chat on friend accept
...
BEGIN
  ...
  INSERT INTO public.calls (caller_id, receiver_id, type, channel_id, status)
...
CREATE INDEX IF NOT EXISTS idx_calls_receiver_id_status ON public.calls(receiver_id, status);
...
CREATE POLICY "Users can view their friendships" ON public.friends FOR SELECT USING (user_id = auth.uid() OR friend_id = auth.uid());
DROP POLICY IF EXISTS "Users can insert friendships" ON public.friends;
CREATE POLICY "Users can insert friendships" ON public.friends FOR INSERT WITH CHECK (user_id = auth.uid());
DROP POLICY IF EXISTS "Users can update their friendships" ON public.friends;
CREATE POLICY "Users can update their friendships" ON public.friends FOR UPDATE USING (user_id = auth.uid() OR friend_id = auth.uid());
DROP POLICY IF EXISTS "Users can delete their friendships" ON public.friends;
CREATE POLICY "Users can delete their friendships" ON public.friends FOR DELETE USING (user_id = auth.uid() OR friend_id = auth.uid());

-- Friend requests policies
DROP POLICY IF EXISTS "Users can view their friend requests" ON public.friend_requests;
CREATE POLICY "Users can view their friend requests" ON public.friend_requests FOR SELECT USING (from_user_id = auth.uid() OR to_user_id = auth.uid());
DROP POLICY IF EXISTS "Users can insert friend requests" ON public.friend_requests;
CREATE POLICY "Users can insert friend requests" ON public.friend_requests FOR INSERT WITH CHECK (from_user_id = auth.uid());
DROP POLICY IF EXISTS "Users can update their friend requests" ON public.friend_requests;
CREATE POLICY "Users can update their friend requests" ON public.friend_requests FOR UPDATE USING (from_user_id = auth.uid() OR to_user_id = auth.uid());
DROP POLICY IF EXISTS "Users can delete their friend requests" ON public.friend_requests;
CREATE POLICY "Users can delete their friend requests" ON public.friend_requests FOR DELETE USING (from_user_id = auth.uid() OR to_user_id = auth.uid());

-- Followers policies
DROP POLICY IF EXISTS "Users can view their followers" ON public.followers;
CREATE POLICY "Users can view their followers" ON public.followers FOR SELECT USING (follower_id = auth.uid() OR followed_id = auth.uid());
DROP POLICY IF EXISTS "Users can follow" ON public.followers;
CREATE POLICY "Users can follow" ON public.followers FOR INSERT WITH CHECK (follower_id = auth.uid());
DROP POLICY IF EXISTS "Users can unfollow" ON public.followers;
CREATE POLICY "Users can unfollow" ON public.followers FOR DELETE USING (follower_id = auth.uid());
...
CREATE POLICY "Users can delete their chats" ON public.chats FOR DELETE USING (created_by = auth.uid());
...
CREATE POLICY "Users can insert chat memberships" ON public.chat_members FOR INSERT WITH CHECK (user_id = auth.uid());
DROP POLICY IF EXISTS "Users can update their chat memberships" ON public.chat_members;
CREATE POLICY "Users can update their chat memberships" ON public.chat_members FOR UPDATE USING (user_id = auth.uid());
DROP POLICY IF EXISTS "Users can delete their chat memberships" ON public.chat_members;
CREATE POLICY "Users can delete their chat memberships" ON public.chat_members FOR DELETE USING (user_id = auth.uid());
...
-- Additional policies for other tables (typing, starred messages, etc.)
DROP POLICY IF EXISTS "View typing in chat" ON public.typing_status;
CREATE POLICY "View typing in chat" ON public.typing_status FOR SELECT USING (
  EXISTS (SELECT 1 FROM public.chat_members WHERE chat_id = typing_status.chat_id AND user_id = auth.uid())
);
DROP POLICY IF EXISTS "Update own typing" ON public.typing_status;
CREATE POLICY "Update own typing" ON public.typing_status FOR UPDATE USING (user_id = auth.uid());
DROP POLICY IF EXISTS "Insert typing" ON public.typing_status;
CREATE POLICY "Insert typing" ON public.typing_status FOR INSERT WITH CHECK (user_id = auth.uid());

DROP POLICY IF EXISTS "Users can manage their starred messages" ON public.starred_messages;
CREATE POLICY "Users can manage their starred messages" ON public.starred_messages FOR ALL USING (user_id = auth.uid()) WITH CHECK (user_id = auth.uid());

DROP POLICY IF EXISTS "Users can view their message statuses" ON public.message_status;
CREATE POLICY "Users can view their message statuses" ON public.message_status FOR SELECT USING (user_id = auth.uid());
DROP POLICY IF EXISTS "Users can update message statuses" ON public.message_status;
CREATE POLICY "Users can update message statuses" ON public.message_status FOR UPDATE USING (user_id = auth.uid());
DROP POLICY IF EXISTS "Users can insert message statuses" ON public.message_status;
CREATE POLICY "Users can insert message statuses" ON public.message_status FOR INSERT WITH CHECK (user_id = auth.uid());

-- Stories policies
DROP POLICY IF EXISTS "Users can view stories" ON public.stories;
CREATE POLICY "Users can view stories" ON public.stories FOR SELECT USING (
  privacy = 'public' OR
  user_id = auth.uid() OR
...
CREATE POLICY "Users can insert their own stories" ON public.stories FOR INSERT WITH CHECK (user_id = auth.uid());
...
CREATE POLICY "Users can view story views" ON public.story_views FOR SELECT USING (viewer_id = auth.uid());
DROP POLICY IF EXISTS "Users can insert story views" ON public.story_views;
CREATE POLICY "Users can insert story views" ON public.story_views FOR INSERT WITH CHECK (viewer_id = auth.uid());

-- Posts and related tables policies
...
CREATE POLICY "Users can insert posts" ON public.posts FOR INSERT WITH CHECK (user_id = auth.uid());
DROP POLICY IF EXISTS "Users can update their posts" ON public.posts;
CREATE POLICY "Users can update their posts" ON public.posts FOR UPDATE USING (user_id = auth.uid());
DROP POLICY IF EXISTS "Users can delete their posts" ON public.posts;
CREATE POLICY "Users can delete their posts" ON public.posts FOR DELETE USING (user_id = auth.uid());
...
CREATE POLICY "Users can comment on posts" ON public.post_comments FOR INSERT WITH CHECK (user_id = auth.uid());
DROP POLICY IF EXISTS "Users can edit their comments" ON public.post_comments;
CREATE POLICY "Users can edit their comments" ON public.post_comments FOR UPDATE USING (user_id = auth.uid());
DROP POLICY IF EXISTS "Users can delete their comments" ON public.post_comments;
CREATE POLICY "Users can delete their comments" ON public.post_comments FOR DELETE USING (user_id = auth.uid());

DROP POLICY IF EXISTS "Users can view post shares" ON public.post_shares;
CREATE POLICY "Users can view post shares" ON public.post_shares FOR SELECT USING (true);
...
CREATE POLICY "Users can delete their shares" ON public.post_shares FOR DELETE USING (user_id = auth.uid());
...
CREATE POLICY "Users can react to posts" ON public.post_reactions FOR INSERT WITH CHECK (user_id = auth.uid());
DROP POLICY IF EXISTS "Users can remove their reactions" ON public.post_reactions;
CREATE POLICY "Users can remove their reactions" ON public.post_reactions FOR DELETE USING (user_id = auth.uid());

DROP POLICY IF EXISTS "Users can view their post reports" ON public.post_reports;
CREATE POLICY "Users can view their post reports" ON public.post_reports FOR SELECT USING (user_id = auth.uid());
DROP POLICY IF EXISTS "Users can report posts" ON public.post_reports;
CREATE POLICY "Users can report posts" ON public.post_reports FOR INSERT WITH CHECK (user_id = auth.uid());
...
CREATE POLICY "Users can tag others" ON public.tags FOR INSERT WITH CHECK (post_id IS NOT NULL AND tagged_user_id IS NOT NULL);
DROP POLICY IF EXISTS "Users can remove tags" ON public.tags;
CREATE POLICY "Users can remove tags" ON public.tags FOR DELETE USING (true);

-- Calls policies
DROP POLICY IF EXISTS "Users can access their calls" ON public.calls;
CREATE POLICY "Users can access their calls" ON public.calls FOR SELECT USING (caller_id = auth.uid() OR receiver_id = auth.uid());
...
CREATE POLICY "Users can read own voice notes" ON public.voice_notes FOR SELECT USING (user_id = auth.uid());
DROP POLICY IF EXISTS "Users can create own voice notes" ON public.voice_notes;
CREATE POLICY "Users can create own voice notes" ON public.voice_notes FOR INSERT WITH CHECK (user_id = auth.uid());
DROP POLICY IF EXISTS "Users can delete own voice notes" ON public.voice_notes;
CREATE POLICY "Users can delete own voice notes" ON public.voice_notes FOR DELETE USING (user_id = auth.uid());
...
CREATE POLICY "Users can view their blocks" ON public.user_blocks FOR SELECT USING (blocker_id = auth.uid() OR blocked_id = auth.uid());
DROP POLICY IF EXISTS "Users can insert blocks" ON public.user_blocks;
CREATE POLICY "Users can insert blocks" ON public.user_blocks FOR INSERT WITH CHECK (blocker_id = auth.uid());
DROP POLICY IF EXISTS "Users can update their blocks" ON public.user_blocks;
CREATE POLICY "Users can update their blocks" ON public.user_blocks FOR UPDATE USING (blocker_id = auth.uid());
DROP POLICY IF EXISTS "Users can delete their blocks" ON public.user_blocks;
CREATE POLICY "Users can delete their blocks" ON public.user_blocks FOR DELETE USING (blocker_id = auth.uid());

-- Solo games policies
DROP POLICY IF EXISTS "Users can view their solo game sessions" ON public.solo_game_sessions;
CREATE POLICY "Users can view their solo game sessions" ON public.solo_game_sessions FOR SELECT USING (user_id = auth.uid());
DROP POLICY IF EXISTS "Users can insert their solo game sessions" ON public.solo_game_sessions;
CREATE POLICY "Users can insert their solo game sessions" ON public.solo_game_sessions FOR INSERT WITH CHECK (user_id = auth.uid());

DROP POLICY IF EXISTS "Users can view their solo leaderboards" ON public.solo_leaderboards;
CREATE POLICY "Users can view their solo leaderboards" ON public.solo_leaderboards FOR SELECT USING (user_id = auth.uid());
DROP POLICY IF EXISTS "Users can insert/update their solo leaderboards" ON public.solo_leaderboards;
CREATE POLICY "Users can insert/update their solo leaderboards" ON public.solo_leaderboards FOR ALL USING (user_id = auth.uid()) WITH CHECK (user_id = auth.uid());
...
CREATE POLICY "Users can insert their achievements" ON public.user_achievements FOR INSERT WITH CHECK (user_id = auth.uid());

DROP POLICY IF EXISTS "Users can view their daily challenges" ON public.daily_challenges;
CREATE POLICY "Users can view their daily challenges" ON public.daily_challenges FOR SELECT USING (user_id = auth.uid());
DROP POLICY IF EXISTS "Users can insert/update their daily challenges" ON public.daily_challenges;
CREATE POLICY "Users can insert/update their daily challenges" ON public.daily_challenges FOR ALL USING (user_id = auth.uid()) WITH CHECK (user_id = auth.uid());

DROP POLICY IF EXISTS "Users can view their spin rewards" ON public.spin_rewards;
CREATE POLICY "Users can view their spin rewards" ON public.spin_rewards FOR SELECT USING (user_id = auth.uid());
DROP POLICY IF EXISTS "Users can insert their spin rewards" ON public.spin_rewards;
CREATE POLICY "Users can insert their spin rewards" ON public.spin_rewards FOR INSERT WITH CHECK (user_id = auth.uid());
...
-- === CHECKLIST: RLS Policies for users table ===
-- Ensure this policy exists to allow all users to be visible in search and suggestions:
-- CREATE POLICY "Users can view all profiles" ON public.users FOR SELECT USING (true);
